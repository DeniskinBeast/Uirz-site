...
interface GovernmentDecreesMonitoringPageState {
...
export default class GovernmentDecreesMonitoringPage extends Component<GovernmentDecreesMonitoringPageState> {
    state: GovernmentDecreesMonitoringPageState = {
    ...
    fetchYearsArrange = (): void => {
        fetch("/api/v1/governmentDecreesYears")
    ...
    fetchLastReport = (): void => {
        fetch("/api/v1/governmentDecreesMonitoringLastReport")
            .then(connectionErrorHandler)
            .then(response => response.json())
    ...
    fetchReportByYear = (year: number): void => {
        fetch(`/api/v1/governmentDecreesMonitoringReportByYear/${year}`)
            .then(connectionErrorHandler)
            .then(response => response.json())
    ...
    fetchReportByMonth = (year: number, month: number): void => {
        fetch(`/api/v1/governmentDecreesMonitoringReportByMonth/${year}/${month}`)
            .then(connectionErrorHandler)
            .then(response => response.json())
            .then(emptyContentErrorHandler)
    ...
    render(): React.ReactElement {
        ...
        return (
            <>
                <Layout title="Мониторинг постановлений правительства Свердловской области"/>
                <Navbar/>
                <div className="content">
                    <Header/>
                    <MonitoringSubNav subNavId="monitoringSubNav" subNavItems={subNavItems}/>
                    <h1 className="text-center page__title">Мониторинг постановлений правительства Свердловской области</h1>
